...
class VaccinationAppointment():
    ...
    def __init__(self, patient_sys_id, date, patient_phone_number):
        self.__alg = "SHA-256"
        self.__type = "DS"
        ...
        if date is not None:
            self.__appointment_date = Date(date).value
        else:
            self.__appointment_date = datetime.timestamp(justnow)
    ...
    def get_appointment_from_date_signature( cls, date_signature ):
        ...
        freezer.start()
        appointment = cls(appointment_record["_VaccinationAppointment__patient_sys_id"],None,
    ...
    def create_appointment_from_json_file( cls, json_file, date):
        ...
        new_appointment = cls(
            patient_sys_id = appointment_parser.json_content[appointment_parser.PATIENT_SYSTEM_ID_KEY],
            patient_phone_number = appointment_parser.json_content[appointment_parser.CONTACT_PHONE_NUMBER_KEY],
            date = date)
