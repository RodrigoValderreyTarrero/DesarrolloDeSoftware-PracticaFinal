...
class TestGetVaccineDate(TestCase):
    ...
    #check the method
        value = my_manager.get_vaccine_date(file_test, "2030-12-01")
        self.assertEqual(value, "8dc9a9ccd14211f17c5f4ccbc3714b80afcb4f68e1c1c70bc51df5aa73f96a77")
    #check store_date
    ...
    def test_get_vaccine_date_no_ok_parameter(self):
        ...
        for file_name,expected_value in param_list_nok:
            with self.subTest(test=file_name):
                file_test = JSON_FILES_TESTS_PATH + file_name
    ...
    def test_get_vaccine_date_no_ok(self):
        ...
        file_test = JSON_FILES_TESTS_PATH + "test_no_ok.json"
        my_manager = VaccineManager()
        file_store_date = AppointmentsJsonStore()
    ...
    def test_get_vaccine_date_no_ok_no_quotes(self):
        """ no quotes , not valid """
        file_test = JSON_FILES_TESTS_PATH + "test_nok_no_comillas.json"
        my_manager = VaccineManager()
        file_store_date = AppointmentsJsonStore()
    ...
    def test_get_vaccine_date_no_ok_data_manipulated( self ):
        ...
        if not os.path.isfile(JSON_FILES_PATH + "store_patient_manipulated.json"):
            shutil.copy(JSON_FILES_TESTS_PATH + "store_patient_manipulated.json",
